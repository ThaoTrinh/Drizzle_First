...
function* createItemSaga({type, payload}) {
  const {data} = yield call(axios.get, 'https://helloacm.com/api/random/?n=10');
...
export default function* rootSaga() {
	yield all([takeEvery(Types.CREATE_ITEM_SAGA, createItemSaga), takeEvery(Types.DELETE_ITEM_SAGA, deleteItemSaga)]);
}
